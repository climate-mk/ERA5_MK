...
Elevation: AWS Terrarium tiles (public, no auth).
Buildings: OpenStreetMap via Overpass API.

Outputs:
  static/data/flood/flood-NNNcm.png   — one RGBA mask per level
  static/data/flood-stats.json        — ha and building counts per level
...
ZOOM = 13
OUT_W, OUT_H = 860, 500
OUT_DIR   = os.path.join(os.path.dirname(__file__), "static", "data", "flood")
STATS_OUT = os.path.join(os.path.dirname(__file__), "static", "data", "flood-stats.json")
TILE_URL  = "https://s3.amazonaws.com/elevation-tiles-prod/terrarium/{z}/{x}/{y}.png"

BUILDINGS_JSON = os.path.join(os.path.dirname(__file__), "static", "data", "buildings.json")
...
def build_grid(zoom):
    ...
    sess.headers["User-Agent"] = "podnebje.kesma.wtf/floodmap-1.0"
...
def fetch_buildings():
    ...
    Generate this file with: python3 scripts/extract_buildings.py
    (or see the osmium extract command in the project README)
    """
    if not os.path.exists(BUILDINGS_JSON):
        print(f"  {BUILDINGS_JSON} not found — skipping building count", flush=True)
        return []
